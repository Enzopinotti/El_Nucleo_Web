  ...
  return (
    ...
    >
      <div className="archive-gallery__intro">
        ...
        <h2 id="archive-title">Cuatro categorías que definían la primera versión</h2>
        <p id="archive-note" className="section-note">
          Este material se conserva como archivo del proyecto original. No representa por sí solo
          un catálogo comercial vigente en 2026.
        </p>
      </div>
      ...
      <div className="archive-gallery__viewer">
        ...
        <div className="archive-gallery__controls" aria-label="Controles de la galería histórica">
          <button type="button" onClick={() => move(-1)} aria-label="Ver categoría anterior">
            <span aria-hidden="true">←</span>
            Anterior
          </button>
          <p className="archive-gallery__status" role="status" aria-live="polite" aria-atomic="true">
            {activeIndex + 1} de {services.length} — {activeService.title}
          </p>
          <button type="button" onClick={() => move(1)} aria-label="Ver categoría siguiente">
            Siguiente
            <span aria-hidden="true">→</span>
          </button>
        </div>
